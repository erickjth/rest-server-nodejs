...
function createHttpError(name, init) {
	function E(message, code) {
		...
		this.code = code;
		this.toJSON = () => ({ code: this.code, message: this.message });

		if (init) {
			init.apply(this, [message, code]);
		}
	}
...
// 500 - General Error
module.exports.GeneralError = createHttpError('GeneralError', function(message) {
	this.message = message || 'GeneralError';
	this.code = HTTP_CODES.GeneralError;
});
